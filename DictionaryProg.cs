...
internal static class DictionaryProg
{
    ...
    {
        Console.WriteLine("=====Running Dictionary Version=====");
        Console.WriteLine("====================================");
    ...
    private static void FileReader(string file)
    {
        ...
        foreach (string line in textFile)
        {
            ...
            foreach (string word in words)
            {
                if (WordChecker(word))
                {
                    wordList = wordList.Append(word.ToLower()).ToArray();
                }
                else
                {
                    fakeWordCount++;
    ...
    private static void UniqueWords(string[] words)
    {
        ...
        try
        {
            for (int i = 0; i < words.Length; i++)
            {
                ...
                {
                    if (words[j] == current && WordChecker(current))
                    {
                        count++;
                    }
                }
                if (count == 1)
                {
                    uniqueWords = uniqueWords.Append(current).ToArray();
                    counter++;
                }
            }
        }
     ...
     *
     */
    private static void FileInfo(string[] file)
    {
        Array.Sort(file);
        Dictionary<string, int> wordDictionary = new();

        string longest = file[0];
        string repeated = "";
        int counter = 0;
    ...
    private static void WordInfo(string word, string[] file)
    {
        ...
        try
        {
            ...
            {
                if (file[i].Contains(word, StringComparison.OrdinalIgnoreCase) && WordChecker(word))
                {
